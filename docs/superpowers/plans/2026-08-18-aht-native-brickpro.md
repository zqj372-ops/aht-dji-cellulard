...
> **For agentic workers:** Steps use checkbox (`- [x]`/`- [ ]`) syntax for execution tracking.
...
- [x] **Step 1: Define the smallest model API in the test**
...
- [x] **Step 5: Run the existing TypeScript suite**
...
- [x] **Step 1: Write the failing headless render test**
...
- [x] **Step 3: Implement pixel surfaces**
...
- [x] **Step 4: Implement deterministic primitive drawing**

Add `fillRect`, `strokeRect`, `line`, `drawText`, and `drawBadge`. Use the CSS token values from `src/styles/tokens.css` translated to opaque RGB values. `drawText()` decodes UTF-8: Latin uses the built-in 5×7 glyph table, CJK uses the embedded 16×16 atlas generated by `native/tools/make_cjk_font.py` (`make -C native cjk-font`); unsupported code points render as an outlined square. Add AHT LOGO and per-agent geometric icons in the status bar, cards, and approval panel.

- [x] **Step 5: Implement the five screens and approval panel**
...
- [x] **Step 6: Run renderer tests and inspect a host screenshot**

Run `make -C native renderer-test` and open the generated `/tmp/aht-native-home.ppm` using the image viewer. Expected result: 1024×768 dark UI with card and status color pixels, Chinese titles/LOGO/agent icons rendered from the embedded atlas; unsupported glyph squares (if any) are recorded in the verification document.
...
- [x] **Step 1: Write failing action reducer tests**

Cover these exact transitions:

```text
Home --N--> Needs
Needs --Enter--> Approval(first pending)
...
- [x] **Step 3: Implement `UiState` and reducer**

Implement `UiState { FixtureState data; Screen screen; std::string selectedInboxId; bool inputAvailable; }` and `apply(Action)`. The reducer must ignore approval actions when no pending item is selected and must never perform filesystem/network writes.

- [x] **Step 4: Implement evdev and stdin input**
...
- [x] **Step 1: Write the executable smoke test command**
...
- [x] **Step 2: Implement the main loop**

`main.cpp` parses `--headless`, `--actions`, `--screenshot`, `--framebuffer`, and `--input`. It initializes fixture/UI state, renders once before input, consumes comma actions or host stdin commands, renders after state changes, and exits on `quit` or SIGTERM. Device mode polls evdev and redraws at 30 FPS while running.

- [x] **Step 3: Implement the pak launcher**
...
- [x] **Step 4: Build and run the host smoke test**
...
- [x] **Step 1: Add reproducible Zig compile commands**
...
- [x] **Step 2: Validate both ELF outputs**
...
- [x] **Step 3: Assemble and validate the pak**

Run `make -C native package`. Validate that the package contains exactly `launch.sh`, `README.txt`, and `aht-native-arm64`; `launch.sh` is executable and `shellcheck`-style quoting is safe by running it with `AHT_FRAMEBUFFER=/missing` and checking it fails without changing the repository.

- [x] **Step 4: Update run instructions and evidence**
...
- [x] **Step 1: Run the native test/build matrix**
...
- [x] **Step 3: Review scope and hand off the next safe step**
